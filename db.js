...
async function selectTabelao(order, item) { 
  ...
  const tabelao = res.rows;
  //console.log('Retorno query tabelão', tabelao);
  return tabelao;
}
...
async function selectOrderItems(order, sequencePK) {
  const client = await connect();
  console.log('*************************************');
  console.log('selectOrderItems sequence', sequencePK);
  ...
  const res = await client.query(`
    ...
    CROSS JOIN ( SELECT SUM(quantity * price) AS frete_produto_total FROM erp.order_items WHERE sequence='${sequencePK}' ) AS tf
