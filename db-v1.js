...
async function selectOrders() {
  ...
  const res = await client.query(`
    SELECT       
      od.sequence, od.order_total, od.item_total, od.discount, od.shipping, od.delivery_company, od.hostname,
...
async function selectOrderItemsKits(order, item, kit) {
  const client = await connect();  
  const res = await client.query(`
    SELECT *, pr.perfil_imposto AS perfil_imposto_id
    FROM erp.product AS pr    
    ...
    WHERE 
      pr.product_id='${kit.product_id}'
  `);

  return orderItems = res.rows;
}

async function selectOrderItems(hostname, sequencePK) {
  const client = await connect();  
  const res = await client.query(`
    ...
    CROSS JOIN ( 
      SELECT SUM(quantity * price) AS frete_produto_total 
      FROM erp.order_items 
      WHERE sequence='${sequencePK}' 
    ) AS tf    
    INNER JOIN erp.product AS pr ON pr.reference_id = it.ref_id    
    INNER JOIN erp.vitrine AS vt ON vt.sku_id = it.id AND vt.product_id = pr.product_id
    INNER JOIN erp.cest AS ct ON ct.cest_id = pr.cest_id
    INNER JOIN erp.perfil_imposto AS pi ON pi.perfil_imposto = pr.perfil_imposto
    WHERE 
      it.sequence='${sequencePK}'
      AND vt.hostname='${hostname}'
  `);
...
async function selectTabelao(order, item) {
  ...
  const res = await client.query(`
    ...
    INNER JOIN erp.icms AS ic ON ic.uf = tb.sigla
...
async function setOrderItems(order, item) {    
  console.log('entrou!');
  ...
  const orderItem = {  
    ...
    "unidadeMedida": "UN",
    "valorUnitario": (item.price/100).toFixed(2),
...
async function setProductKit(order, item) {
  const client = await connect();  
  let orderItem = null;
  let itemsObject = [];  
  ...
  const res = await client.query(`
    SELECT *
    FROM erp.product_kit
    WHERE 
      kit_product_id = '${item.product_id}';
  `);

  const kits = res.rows;

  for (const kit of kits) {    
    orderItem = await selectOrderItemsKits(order, item, kit);
    itemsObject.push(orderItem);
  }
...
async function payloadSend(order, orderItems) {
  ...
  let orderItem, kit = null;
  let itemsObject = [];  
  
  for (const item of orderItems) {    
    if ( item.is_kit === true ) {
      kit = await setProductKit(order, item);      
      orderItem = await setOrderItems(order, kit);
    } else {
      //orderItem = await setOrderItems(order, item);      
    }

    itemsObject.push(orderItem);
  };    
    
  orderWeight = 0; /* calcular peso */
